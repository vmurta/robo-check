...
// Do COARSENING_FACTOR tests per thread
// only top level box
// used shared memory to enqueue 
// check N configurations for full collision 
#define COARSENING_FACTOR 29
#define BLOCK_SIZE 32
#define NUM_CONFS_PER_BLOCK (COARSENING_FACTOR * BLOCK_SIZE)
__global__ void d_obb_naive_coursed ( const Eigen::Matrix3f* pR_obs, const Eigen::Vector3f* pT_obs,
                                const Eigen::Matrix3f* pR_rob, const Eigen::Vector3f* pT_rob,
                                const Eigen::Vector3f* pRob_dim, const Eigen::Vector3f* pObs_dim,
                                const Eigen::Matrix3f* pRob_conf_rot, const Eigen::Vector3f* pRob_conf_trans,
                                bool* pdisjoint) {    
    ...
    __shared__ Eigen::Matrix3f shared_R_conf[NUM_CONFS_PER_BLOCK];
    __shared__ Eigen::Vector3f shared_T_conf[NUM_CONFS_PER_BLOCK];
    uint32_t local_conf_idx = threadIdx.x;
    uint32_t global_conf_idx = blockIdx.x * NUM_CONFS_PER_BLOCK + local_conf_idx;
    for (; local_conf_idx < NUM_CONFS_PER_BLOCK; local_conf_idx += BLOCK_SIZE){
        shared_R_conf[local_conf_idx] = pRob_conf_rot[global_conf_idx];
        global_conf_idx += BLOCK_SIZE;
    }
    
    local_conf_idx = threadIdx.x;
    global_conf_idx = blockIdx.x * NUM_CONFS_PER_BLOCK + local_conf_idx;
    for (; local_conf_idx < NUM_CONFS_PER_BLOCK; local_conf_idx += BLOCK_SIZE){
    ...
    __shared__ uint32_t overlap_indices[COARSENING_FACTOR * BLOCK_SIZE]; // arbitrarily chosen buffer size, should experiment with this
    if (threadIdx.x == 0){
        overlap_count = 0;
    }
    __syncthreads();
    ...
    // __shared__ Eigen::Matrix3f overlap_R_conf[COARSENING_FACTOR];
    // __shared__ Eigen::Vector3f overlap_T_conf[COARSENING_FACTOR];
    
    local_conf_idx = threadIdx.x;
    global_conf_idx = blockIdx.x * BLOCK_SIZE * COARSENING_FACTOR + local_conf_idx;
    ...
    uint32_t global_conf_offset = blockIdx.x * BLOCK_SIZE * COARSENING_FACTOR;
    // if (local_conf_idx == 0) {
...
void broad_naive_1() {
    ...
    TIMEIT("launching the broad_naive kernel", 
...
void broad_coarsened_shared_mem() {
    // Similar setup as broad_naive_1 but using d_obb_naive_coursed kernel
        // Load Robot and Obstacle BVH
    OBB_soa rob_BVH = hierarchy_from_mesh("/home/victor/Projects/robo-check/data/models/alpha1.0/robot.obj");
    OBB_soa obs_BVH = hierarchy_from_mesh("/home/victor/Projects/robo-check/data/models/alpha1.0/obstacle.obj");
    ...
    const int confs_per_block = COARSENING_FACTOR * blockSize;
    const int gridSize = (num_confs + confs_per_block - 1) / confs_per_block; // ceil division

    TIMEIT("launching the coarsened kernel", 
        d_obb_naive_coursed<<<gridSize, blockSize>>>(d_R_obs, d_T_obs, d_R_rob, d_T_rob, d_Obs_dim, d_Rob_dim, d_Rob_conf_rot, d_Rob_conf_trans, pdisjoint);
        cudaDeviceSynchronize();
        checkCudaMem(cudaGetLastError());
        // Copy result back to host (num_confs * sizeof(bool))
        checkCudaMem(cudaMemcpy(disjoint, pdisjoint, num_confs * sizeof(bool), cudaMemcpyDeviceToHost));
